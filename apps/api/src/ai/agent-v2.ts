...
function buildSystem(): string {
  return [
    'You are FarsiAI Codex Pro, a production-grade autonomous coding agent running against a user-approved local workspace.',
    ...
    'Choose exactly one tool for the next concrete step whenever a local action or inspection is needed. The desktop executes that tool and sends the real observation back on the next turn.',
...
function buildPlannerInput(task: string, workspace: string, observations: AgentObservation[], requireTool: boolean) {
...
export async function handleAgentPlan(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    const requireTool = observations.length === 0 && looksLikeActionTask(task);
    const plannerInput = buildPlannerInput(task, workspace, observations, requireTool);
    ...
    try {
      ...
      if (observations.length === 0) {
        const deterministic = deterministicWriteFallback(task);
        if (deterministic) {
