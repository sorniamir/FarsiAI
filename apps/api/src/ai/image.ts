...
async function geminiInteractions(
  ...
  if (reference) input.push({ type: 'image', mime_type: reference.mimeType, data: reference.base64 });
...
async function runNanoBanana(
  ...
  const configured = env.NANO_BANANA_MODEL?.trim() || DEFAULT_NANO_BANANA_MODEL;
  const models = configured === FALLBACK_NANO_BANANA_MODEL
    ? [configured]
    : [configured, FALLBACK_NANO_BANANA_MODEL];
...
async function workersStreamImage(
  env: Env,
  model: string,
  prompt: string,
): Promise<{ image: string; provider: string } | null> {
  try {
    ...
    return {
      image: await bytesToDataUrl(result, 'image/png'),
      provider: model,
    };
  } catch (error) {
    console.warn(JSON.stringify({
      event: 'workers_image_fallback_failed',
      model,
      message: error instanceof Error ? error.message : 'unknown_workers_image_error',
    }));
    return null;
  }
}
...
export async function runImage(
  ...
  for (const model of [IMAGE_FALLBACK_MODEL, IMAGE_LAST_RESORT_MODEL]) {
    const fallback = await workersStreamImage(env, model, finalPrompt);
    if (fallback) {
      return {
  ...
  throw new Error('IMAGE_PROVIDERS_UNAVAILABLE');
}
